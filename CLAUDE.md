...
- **F3/T18 concluída (21/07/2026)**: fluxo único de publicação de produção (ADR-0041), workflow `.github/workflows/deploy.yml` disparado só por push em `master` (mais `workflow_dispatch` de recuperação), `concurrency` sem cancelamento para nunca reordenar migration/deploy/smoke de duas publicações. Sequência: testes/typecheck/build completos contra o Supabase **local** (nunca o de produção) → `supabase db push` aditivo contra o Postgres de produção → verificação de drift (`pnpm --filter @farejo/web verify:schema`, `apps/web/test/verify-production-schema.mts`) que confirma via `pg_roles`/`information_schema.views`/`pg_proc`/`pg_class.relrowsecurity`/`storage.buckets`/`has_table_privilege`/`has_function_privilege`/`has_column_privilege` que as seis roles, as seis views de `web_read`, as nove funções, as dez tabelas com RLS e uma amostra de alto risco dos GRANTs (leitura das views/execução das funções por `farejo_web`, execução de `activation.resolve_destination`/`record_activation`, execução de `curation.apply_alias_merge`/`verify_alias_merge`, e o grant de coluna `logo_url`/`logo_hash` de `farejo_logo_writer`) batem com o que as migrations já mescladas declaram, sem depender de "push sem erro" como único sinal — testado ao vivo contra o Postgres local revogando grants reais e confirmando que o passo detecta a queda → build Vercel via `vercel pull`/`vercel build --prod` e `vercel deploy --prebuilt --prod` (o mesmo artefato validado, sem rebuild divergente na Vercel) → smoke pós-deploy (`pnpm --filter @farejo/web test:smoke:production`, `apps/web/test/smoke-production.mts`) batendo `/`, busca, `/sitemap.xml`, `/robots.txt`, detalhe de loja (primeiro slug do sitemap com oferta ativa), `/go/[slug]/[platformId]` frio+morno com p50/p95 registrados (referência ADR-0032) e a invalidação HMAC rejeitando sem assinatura e aceitando com assinatura válida — nunca abre conexão de banco. `apps/web/vercel.json` desliga o auto-deploy da integração Git da Vercel só para `master` (`git.deploymentEnabled.master=false`), preservando Preview automático para outras branches sem banco remoto (ADR-0037); `regions`/`preferredRegion` fixados em `gru1` (São Paulo) — **confirmado em 21/07/2026**: o projeto Supabase é `sa-east-1` (São Paulo), mesma localidade. Rota `/go/[storeSlug]/[platformId]` ganhou `runtime="nodejs"` explícito (documenta a exigência do driver `pg`, non-negotiable mesmo que o padrão do Next mude).
- **Publicação validada ao vivo e corrigida (21/07/2026, ADR-0056)**: o `deploy.yml` rodou ponta a ponta contra o projeto Vercel e o Supabase de produção reais. A sequência deploy → smoke → rollback do desenho original tinha **dois defeitos que só aparecem quando algo falha** e ambos ocorreram: (a) o smoke mirava `FAREJO_SITE_URL` — o domínio — e não o deployment recém-criado, então bastava a produção estar fixada por um `vercel rollback` (a Vercel para de mover o domínio até o pin ser desfeito) para o smoke medir o artefato **antigo**, reprovar, disparar outro rollback e travar o workflow num ciclo do qual ele não sai sozinho; (b) o plano gratuito da Vercel só permite voltar **um** deployment — a segunda tentativa devolve HTTP 402. Agora a publicação é **encenada**: `vercel deploy --prebuilt --prod --skip-domain` cria o deployment de produção sem mover o domínio, o smoke roda contra `steps.deploy.outputs.url` (com `x-vercel-protection-bypass`/`VERCEL_AUTOMATION_BYPASS_SECRET`, porque a Deployment Protection responde 302 em URL de deployment — só o domínio é público), e só depois de aprovado `vercel promote` torna o artefato público. O público nunca vê artefato não verificado e não existe mais o que reverter: falha em qualquer passo deixa a produção intacta. O passo de rollback saiu. `FAREJO_SITE_URL` deixou de ser secret exigido pelo workflow (nenhum passo o lê; quem lê o domínio é a app em runtime, pela env var da Vercel).
- **ADR-0055 (21/07/2026) — TLS das conexões `pg`**: o certificado do Postgres/pooler do Supabase não encadeia até uma CA pública, e o bundle padrão do Node recusa a cadeia (`SELF_SIGNED_CERT_IN_CHAIN`). Isso nunca apareceu antes porque o scraper escreve em produção por `supabase-js` (HTTPS/PostgREST, CA pública); os seis `Pool` do `pg` só tinham falado com o stack local, que não usa TLS. Fronteira única por app (`apps/scraper/src/postgresPool.ts`, `apps/web/src/lib/postgres-pool.ts` — não em `shared`, que a ADR-0002 mantém sem `process.env`), CA em `FAREJO_SUPABASE_CA_CERT` como **PEM inteiro** em vez de arquivo (mesmo valor vale em Actions e em função serverless, sem `outputFileTracingIncludes`). Duas armadilhas viraram falha explícita: connection string **não pode conter `sslmode`** (`pg` faz `Object.assign({}, config, parse(connectionString))` e `pg-connection-string` cria `ssl = {}` ao ver `sslmode` — o parse vence e o CA seria descartado em silêncio) e host remoto sem CA é recusado em vez de degradar para conexão não verificada. Só `localhost`/`127.0.0.1`/`::1` dispensam TLS.
...
- **Publicação:** um único Supabase de produção; previews sem banco remoto; workflow coordena migrations aditivas, deploy e smoke tests. Usar Node/Next/React estáveis mais recentes compatíveis com Vercel. **A publicação é encenada (ADR-0056): verifica o artefato antes de promovê-lo, nunca o contrário.**
- **Toda conexão `pg` verifica o certificado do servidor (ADR-0055)** via `FAREJO_SUPABASE_CA_CERT`. Nenhuma connection string pode conter `sslmode` — ele descarta o CA em silêncio.
...
5. **Investigar por que a ingestão de logos publica zero logos** — única pendência que bloqueia a meta de 95% (ADR-0043). Detalhes na seção abaixo.
6. ~~Configurar o Environment `logos` e o `production`, criar o projeto na Vercel~~ — **concluído em 21/07/2026**. Ambos os Environments estão populados; a produção está no ar em `https://farejo.vercel.app` servindo o artefato `--prebuilt` validado. Secrets novos desde então: `FAREJO_SUPABASE_CA_CERT` (**secret de repositório**, não de Environment — é certificado público, e uma cópia única evita divergência silenciosa) e `VERCEL_AUTOMATION_BYPASS_SECRET` (Environment `production`). O domínio `farejo.com.br` **ainda não aponta para a Vercel** (hoje serve algo da Hostinger, que responde 200 com HTML em qualquer caminho — nunca use esse domínio para verificar o site).

### Logos: ingestão funciona, download não (21/07/2026)

O Environment `logos` está configurado e o `logos:ingest` roda **verde**, mas publica **zero** logos:

```
[logos] 1044 candidatas, 0 atualizadas, 0 falharam, 1044 seguem no fallback visual
[logos] fontes rejeitadas por classe: network_or_http=2182
⚠️ [logo-coverage] 0/1037 lojas elegíveis com logo final (0.0%, meta ADR-0043 >= 95%)
```

O verde é o desenho funcionando (falha de fonte vira fallback honesto, nunca derruba o job), não sucesso. Ambas as roles `pg` conectam e leem normalmente — o problema é 100% dos downloads falhando, em todas as cinco plataformas, o que exige causa sistemática.

**Descartado por teste, não por suposição:** User-Agent de bot (`DEFAULT_USER_AGENT` em `net.ts` contraria a convenção de "UA de browser real", mas baixou um logo do Inter com 200/241 KB); disponibilidade do host; IPv6 sendo escolhido por `records[0]` com `verbatim: true` (o host do Inter só tem registro A).

**Não testado:** o caminho `undiciFetch` com `Agent` de `lookup` fixado no IP validado, sobre HTTPS (`apps/scraper/src/logos/net.ts`).

**Evidência decisiva ainda não coletada** — o ingestor grava o erro real de cada fonte:

```sql
select rejection_reason, count(*) from store_logo_sources
where rejection_reason is not null group by 1 order by 2 desc limit 20;
```

`HTTP 403`/`404` aponta para o lado do CDN; erro de socket ou TLS aponta para o `Agent` com lookup fixado.
