...
import { catalogHref, getCatalogPage, getEligibleStoreSlugs } from "@/lib/catalog";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const detailPages = (await getEligibleStoreSlugs()).map((slug) => ({
    url: new URL(`/loja/${encodeURIComponent(slug)}`, siteUrl).toString(),
  }));
