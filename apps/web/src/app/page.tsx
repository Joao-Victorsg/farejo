  ...
  return (
    <PageFrame>
      <main id="conteudo" tabIndex={-1}>
        <section className="border-b border-[#ece9e2] bg-[#faf9f5]"><div className="mx-auto max-w-[1160px] px-5 py-16 sm:px-8 sm:py-24"><div className="grid items-center gap-12 lg:grid-cols-[minmax(0,1fr)_auto]"><div><p className="font-mono text-xs font-medium tracking-[0.14em] text-[#1c7a4d]">{editorial.home.eyebrow}</p><h1 className="mt-5 max-w-2xl text-4xl font-bold leading-[1.03] tracking-[-0.055em] sm:text-6xl">{editorial.home.title}</h1><p className="mt-6 max-w-lg text-lg leading-8 text-[#5b5f56]">{editorial.home.description}</p><HeroSearch placeholder={editorial.home.searchPlaceholder} query={request.query} /></div><dl className="flex gap-4"><div className="flex-1 rounded-2xl border border-[#ece9e2] bg-white px-7 py-8 text-center"><dd className="font-numbers text-5xl font-bold leading-none text-[#1c7a4d]">{formatHeroStoreCount(catalog.total)}</dd><dt className="mt-2 text-sm text-[#5b5f56]">lojas</dt></div><div className="flex-1 rounded-2xl border border-[#ece9e2] bg-white px-7 py-8 text-center"><dd className="font-numbers text-5xl font-bold leading-none text-[#1c7a4d]">5</dd><dt className="mt-2 text-sm text-[#5b5f56]">plataformas</dt></div></dl></div></div></section>
        <section id="catalogo" className="mx-auto max-w-[1160px] px-5 py-16 sm:px-8"><p className="font-mono text-xs font-medium tracking-[0.13em] text-[#1c7a4d]">CATÁLOGO</p><h2 className="mt-3 text-3xl font-bold tracking-[-0.04em]">{request.query ? `Resultados para “${request.query}”` : "Todas as lojas"}</h2><p className="mt-4 max-w-2xl leading-7 text-[#5b5f56]">{request.sort === "platforms" ? "Ordenadas pela quantidade de plataformas com cashback verificado." : request.sort === "cashback" ? "Percentuais aparecem antes de valores fixos; cada unidade mantém sua própria ordem." : "Ordenadas pelo nome canônico da loja."}</p><div className="mt-8 flex flex-wrap items-center justify-between gap-4"><CatalogControls query={request.query} sort={request.sort} /><InterToggle compact /></div>{catalog.total === 0 ? request.query ? <div className="mt-8 rounded-2xl border border-[#e0ddd4] bg-[#faf9f5] p-6"><h3 className="text-xl font-bold">Nenhuma loja com cashback disponível foi encontrada.</h3><p className="mt-2 text-[#5b5f56]">Tente outro nome ou limpe a busca para ver o catálogo completo.</p><Link className="mt-4 inline-flex font-semibold text-[#1c7a4d]" href="/#catalogo">Limpar busca</Link></div> : <div className="mt-8 rounded-2xl border border-[#e0ddd4] bg-[#faf9f5] p-6"><h3 className="text-xl font-bold">O catálogo está temporariamente vazio.</h3><p className="mt-2 text-[#5b5f56]">Isso pode indicar uma anomalia nos dados. Tente novamente em alguns instantes.</p></div> : pageOutOfRange ? <PageNotFound /> : <><div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{catalog.items.map((store) => <CatalogCard key={store.slug} store={store} />)}</div><Pagination request={request} totalPages={catalog.totalPages} /><p aria-live="polite" className="mt-5 text-center text-sm text-[#5b5f56]">Página {catalog.page} de {catalog.totalPages} · {catalog.total} lojas elegíveis · {CATALOG_PAGE_SIZE} por página</p></>}</section>
      </main>
    </PageFrame>
  );
}
