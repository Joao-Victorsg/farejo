...
function Pagination({ request, totalPages }: { request: CatalogRequest; totalPages: number }) {
  ...
  return (
    <nav aria-label="Paginação do catálogo" className="mt-10 flex flex-wrap items-center justify-center gap-2">
      {request.page > 1 ? <Link className="inline-flex min-h-10 items-center gap-1 rounded-lg border border-[#e0ddd4] px-3 text-sm font-semibold" href={hrefForCatalog({ ...request, page: request.page - 1 })}><ArrowLeft aria-hidden="true" size={16} />Anterior</Link> : <span aria-disabled="true" className="inline-flex min-h-10 items-center gap-1 rounded-lg border border-[#ece9e2] px-3 text-sm text-[#9a9d94]"><ArrowLeft aria-hidden="true" size={16} />Anterior</span>}
      ...
      {request.page < totalPages ? <Link className="inline-flex min-h-10 items-center gap-1 rounded-lg border border-[#e0ddd4] px-3 text-sm font-semibold" href={hrefForCatalog({ ...request, page: request.page + 1 })}>Próxima<ArrowRight aria-hidden="true" size={16} /></Link> : <span aria-disabled="true" className="inline-flex min-h-10 items-center gap-1 rounded-lg border border-[#ece9e2] px-3 text-sm text-[#9a9d94]">Próxima<ArrowRight aria-hidden="true" size={16} /></span>}
    </nav>
  );
}
  ...
  return (
    <PageFrame>
      <main id="conteudo" tabIndex={-1}>
        ...
        <section id="catalogo" className="mx-auto max-w-[1160px] px-5 py-16 sm:px-8"><p className="font-mono text-xs font-medium tracking-[0.13em] text-[#1c7a4d]">CATÁLOGO</p><h2 className="mt-3 text-3xl font-bold tracking-[-0.04em]">{request.query ? `Resultados para “${request.query}”` : "Todas as lojas"}</h2><p className="mt-4 max-w-2xl leading-7 text-[#5b5f56]">{request.sort === "platforms" ? "Ordenadas pela quantidade de plataformas com cashback verificado." : request.sort === "cashback" ? "Percentuais aparecem antes de valores fixos; cada unidade mantém sua própria ordem." : "Ordenadas pelo nome canônico da loja."}</p><CatalogControls query={request.query} sort={request.sort} />{catalog.total === 0 ? request.query ? <div className="mt-8 rounded-2xl border border-[#e0ddd4] bg-[#faf9f5] p-6"><h3 className="text-xl font-bold">Nenhuma loja com cashback disponível foi encontrada.</h3><p className="mt-2 text-[#5b5f56]">Tente outro nome ou limpe a busca para ver o catálogo completo.</p><Link className="mt-4 inline-flex font-semibold text-[#1c7a4d]" href="/#catalogo">Limpar busca</Link></div> : <div className="mt-8 rounded-2xl border border-[#e0ddd4] bg-[#faf9f5] p-6"><h3 className="text-xl font-bold">O catálogo está temporariamente vazio.</h3><p className="mt-2 text-[#5b5f56]">Isso pode indicar uma anomalia nos dados. Tente novamente em alguns instantes.</p></div> : pageOutOfRange ? <PageNotFound /> : <><div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{catalog.items.map((store) => <CatalogCard key={store.slug} store={store} />)}</div><Pagination request={request} totalPages={catalog.totalPages} /><p aria-live="polite" className="mt-5 text-center text-sm text-[#5b5f56]">Página {catalog.page} de {catalog.totalPages} · {catalog.total} lojas elegíveis · {CATALOG_PAGE_SIZE} por página</p></>}</section>
      </main>
    </PageFrame>
  );
}
