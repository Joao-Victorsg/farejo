...
interface PendingRow {
  ...
  previous_is_upto: boolean | null;
}

function toTransition(row: PendingRow): PendingTransition {
  return {
...
export async function sendPendingAvisos(pool: NotifierPool, transport: AvisoTransport): Promise<AvisoRunReport> {
  ...
  const pending = await pool.query<PendingRow>("select * from alerts.pending_avisos($1)", [watermark]);

  const avisos = buildAvisos(pending.rows.map(toTransition));
