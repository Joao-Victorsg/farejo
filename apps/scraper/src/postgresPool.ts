...
/**
 * Fronteira única de TLS das conexões `pg` do scraper (ADR-0055).
 *
 ...
 *
 * Espelhado em `apps/web/src/lib/postgres-pool.ts`: a ADR-0002 mantém `packages/shared` como
 * domínio puro que nunca lê `process.env`, e configuração de I/O é I/O.
 */
...
export function resolvePostgresSsl(
  ...
  throw new Error(
    "FAREJO_SUPABASE_CA_CERT ausente para uma conexão Postgres remota (ADR-0055). Configure o secret do Environment antes de conectar.",
  );
}
